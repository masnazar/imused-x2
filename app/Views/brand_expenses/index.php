...
<?= $this->section('content') ?>
  ...
  <div class="d-flex align-items-center justify-content-between page-header-breadcrumb gap-2 mb-3">
    <div>
      ...
      <h1 class="page-title fw-medium fs-18 mb-0"><i class="ti ti-list"></i> Daftar Brand Expenses</h1>
    </div>
  </div>
  ...
  <div class="card custom-card">
    <div class="card-body table-responsive">
      <table id="brandExpensesTable" class="table table-bordered text-nowrap">
        <thead>
          <tr>
            <th>No</th><th>Tanggal</th><th>Brand</th><th>COA</th><th>Platform</th>
            <th class="text-end">Jumlah</th><th>Tipe</th><th>Diproses Oleh</th><th>Aksi</th>
          </tr>
        </thead>
        <tbody></tbody>
      </table>
...
<script>
$(function(){
  let csrfName = '<?= csrf_token() ?>', csrfHash = '<?= csrf_hash() ?>';
  const table = $('#brandExpensesTable').DataTable({
    ...
    columns:[
      ...
      { data:'date' },
      { data:'brand_name' },
      { data:null, render:(v,t,r)=>r.coa_code + ' – ' + r.coa_name },
      { data:null, render:(v,t,r)=> r.platform_code? r.platform_code+' – '+r.platform_name : '-' },
      { data:'amount', className:'text-end', render:$.fn.dataTable.render.number('.',',',2) },
      { data:'type' },
      { data:'processed_by_name' },
      { data:'id', orderable:false, className:'text-center',
        render:id=>`
          <a href="<?= site_url('brand-expenses/edit') ?>/${id}" class="btn btn-sm btn-warning me-1">Edit</a>
