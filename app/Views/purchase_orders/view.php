    ...
    <div>
        <nav>
            <ol class="breadcrumb mb-1">
                ...
                <li class="breadcrumb-item">
                    <a href="<?= base_url('purchasing') ?>">Manajemen Pembelian</a>
                </li>
                <li class="breadcrumb-item">
                    <a href="<?= base_url('purchase-orders') ?>">Purchase Orders</a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">
                    Detail PO #<?= esc($po['po_number']) ?>
                </li>
            </ol>
        </nav>
                ...
                <div class="h5 mb-0 d-sm-flex d-block align-items-center">
                    ...
                    <div class="ms-sm-2 ms-0 mt-sm-0 mt-2">
                        <div class="h6 fw-medium mb-0">
                            PURCHASE ORDER : <span class="text-primary">#<?= esc($po['po_number']) ?></span>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="border p-3 rounded-3">
                            <div class="row">
                                ...
                                <div class="col-6">
                                    ...
                                    <span class="badge bg-<?= $po['status'] == 'completed' ? 'success' : 'warning' ?>-transparent">
                                        <?= esc($po['status']) ?>
                                    </span>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    <?php foreach ($po['products'] as $index => $product): ?>
                                        <tr>
                                            ...
                                            <!-- Qty PO -->
<td class="text-end"><?= number_format($product['quantity'], 0, ',', '.') ?> pcs</td>

<!-- Diterima -->
<td class="text-end"><?= number_format($product['received_quantity'], 0, ',', '.') ?> pcs</td>

<!-- Sisa -->
<td class="text-end"><?= number_format($product['remaining_quantity'], 0, ',', '.') ?> pcs</td>
...
<!-- Tabel Histori Penerimaan -->
<div class="col-xl-12">
    <div class="card custom-card">
        <div class="card-header">
            <div class="card-title">
                <i class="ri-history-line me-2"></i> Histori Penerimaan Produk
            </div>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered text-nowrap border">
                    <thead class="table-light">
                        <tr>
                            <th class="text-center" width="50">#</th>
                    ...
                    <tbody>
                        <?php 
                        $totalReceived = 0;
                        foreach ($receiptLogs as $index => $log): 
                            $totalReceived += $log['received_quantity'];
                        ?>
                            <tr>
                                <td class="text-center"><?= $index + 1 ?></td>
                                <td><?= esc($log['nomor_surat_jalan']) ?></td>
                                <td><?= date('d M Y', strtotime($log['received_at'])) ?></td>
                                <td><?= esc($log['warehouse_name']) ?></td>
                                <td><?= esc($log['sku']) ?></td>
                                <td><?= esc($log['product_name']) ?></td>
                                <!-- Qty Diterima -->
<td class="text-end"><?= number_format($log['received_quantity'], 0, ',', '.') ?> pcs</td>
                                <td><?= date('d M y - H:i', strtotime($log['received_at'])) ?></td>
                            </tr>
                        <?php endforeach; ?>
                        
                        <?php if (empty($receiptLogs)): ?>
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4">
                                    <i class="ri-inbox-line me-2"></i> Belum ada riwayat penerimaan
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                    ...
                    <tfoot class="border-top">
                        <tr>
                            <td colspan="6" class="text-end fw-medium">Total Diterima</td>
                            <td class="text-end fw-medium text-success">
    <?= number_format($totalReceived, 0, ',', '.') ?> pcs
</td>
                            <td></td>
                        </tr>
                    </tfoot>
